...
# Hyperparameters
####################
learning_rate = 1e-4    # TODO: Change here as you see fit
batch_size = 32         # TODO: Change here as you see fit
epochs = 20              # TODO: Change here as you see fit
...
# Load dataset
full_dataset = ObjectDataset(labels)
...
# TODO: Create a model
model = MultiTaskModel(num_classes=len(full_dataset.idx_to_class)).to(device)
print(model)
...
model = MultiTaskModel(num_classes=len(full_dataset.idx_to_class)).to(device)
model.load_state_dict(torch.load('model_best_vloss.pth', map_location=device))
model.eval()
...
test_preds = model(test_images.to(device))[0].argmax(1)
test_bboxes_pred = model(test_images.to(device))[1]
